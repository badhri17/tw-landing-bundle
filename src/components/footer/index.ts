...
export default class GrowthFooter extends GrowthElement {
    ...
    const candidates: Array<SocialLink | null> = [
      this._social(
        ...
        html`<svg viewBox="0 0 24 24" aria-hidden="true" fill="currentColor">
          <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25h6.826l4.713 6.231 5.45-6.231Zm-1.161 17.52h1.833L7.084 4.126H5.117L17.083 19.77Z" />
        </svg>`
      ),
      this._social(
        "Snapchat",
      ...
      this._social(
        ...
        c.tiktok_url,
        html`<svg viewBox="0 0 24 24" aria-hidden="true" fill="currentColor">
          <path d="M15.57 2.25h-3.39v13.18a3.05 3.05 0 1 1-2.63-3.02V8.98a6.44 6.44 0 1 0 6.02 6.42V8.7a8.04 8.04 0 0 0 4.7 1.5V6.81a4.7 4.7 0 0 1-4.7-4.56Z" />
        </svg>`
      ),
      this._social(
        "Instagram",
        c.instagram_url,
        html`<svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2">
  ...
  render() {
    ...
    const spacing = this._pickValue<FooterSpacing>(
      c.spacing,
      "comfortable"
    );
    const sectionStyle = [
    ...
    return html`
      ...
      >
        <div class="inner">
          ...
          ${copyright
            ? html`<p class="copyright">${copyright}</p>`
            : nothing}
        </div>
      </footer>
    `;
